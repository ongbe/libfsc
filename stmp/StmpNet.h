...
public:
	...
	void sendEnd(uint dtid, ushort ret, Message* end); /* STMP-END. */
	void sendUni(uint stid, Message* end);
public:
	...
	static stmp_pdu* encodeEnd(uint dtid, ushort ret, string* end); /*  encode-STMP-END. */
	static stmp_pdu* encodeEndWithPart(uint dtid, ushort ret, uchar* dat, int ofst, int len); /*  encode-STMP-END. */
