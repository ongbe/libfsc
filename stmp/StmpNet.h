...
public:
	...
	void sendUni(uint stid, uint* sid /* 用于subscribe/publish. */, Message* uni); /* STMP-UNI. */
public:
...
public:
	void subscribe(const string* subsribe, uint sid); /* 将自己添加到消息订阅服务. */
	void unSubscribe(); /* 将自己从消息订阅服务上卸下. */
	static void publishMsg(const string* root, const string* path, Message* publish); /* 发布订阅服务上的消息. */
public:
